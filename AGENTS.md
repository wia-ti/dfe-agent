...
- [x] **Empacotamento do agente `dfe-agent` como `@wiati/dfe-agent` no npm registry** (BLOQUEANTE B14.1). Pacote `@wiati/dfe-agent@0.1.0` publicado em https://www.npmjs.com/package/@wiati/dfe-agent em 2026-08-27. Origem do nome: usuario npm `wiati` (sem hifen) eh o owner do publisher; tentativas anteriores usaram `@dfe-agent/` (org inexistente) e `@wia-ti/` (com hifen, username diferente) — corrigido para `@wiati/`. 17 rounds de CI debugging (sync step, tsx invocation, YAML indent, missing commits, scope mismatch, 2FA, granular tokens) culminaram em Trusted Publishing via OIDC (npm policy 2024+). **Pipeline final**: `git tag packages-v*.*.* && git push origin` → GitHub Actions OIDC token → npm registry valida contra Trusted Publisher configurado em https://www.npmjs.com/settings/wiati/profile. NAO precisa de NPM_TOKEN secret nem 2FA bypass (que foi removido pelo npm). Workflow file: `.github/workflows/publish-npm.yml` com `permissions: id-token: write` + `npm publish --access public --provenance`. Matriz CI: ubuntu 20.x + 22.x × 4 jobs (Sync + Drift + Build + Lint + Test, todos PASS). Pacote inclui 65 files, 30.9 kB tarball, 102 kB unpacked, signatarias OIDC.
