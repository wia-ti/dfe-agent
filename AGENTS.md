...
- **Drift-check local**: `cd packages/dfe-agent && npm run drift-check` (substitui o gate CI removido em Sprint 18). Drift detectado = commit bloqueado localmente pelo humano (gate B.1 vira responsabilidade do gate de commit).
...
## Decisoes resolvidas (Sprint 14)

- [x] **Empacotamento do agente `dfe-agent` como `@wiati/dfe-agent` no npm registry** (BLOQUEANTE B14.1). Pacote `@wiati/dfe-agent@0.1.0` publicado em https://www.npmjs.com/package/@wiati/dfe-agent em 2026-08-27. Origem do nome: usuario npm `wiati` (sem hifen) eh o owner do publisher; tentativas anteriores usaram `@dfe-agent/` (org inexistente) e `@wia-ti/` (com hifen, username diferente) — corrigido para `@wiati/`. 17 rounds de CI debugging (sync step, tsx invocation, YAML indent, missing commits, scope mismatch, 2FA, granular tokens) culminaram em Trusted Publishing via OIDC (npm policy 2024+). **Pipeline final**: `git tag packages-v*.*.* && git push origin` → GitHub Actions OIDC token → npm registry valida contra Trusted Publisher configurado em https://www.npmjs.com/settings/wiati/profile. NAO precisa de NPM_TOKEN secret nem 2FA bypass (que foi removido pelo npm). Workflow file (removido em Sprint 18): vide "Decisoes resolvidas (Sprint 18)". Pacote inclui 65 files, 30.9 kB tarball, 102 kB unpacked, signatarias OIDC.

- [x] **Plano `PLAN_SPRINT14.md` (1090 linhas) + feature plan `PLAN_FEATURE_dfe-agent-npm-package.md` (500 linhas) commitados** (gate Fase 1 do `/feature`). 10 decisoes arquiteturais (D1-D10) resolvidas: mono-repo `packages/dfe-agent/` (D1), scope `@wiati/dfe-agent` (D2 — corrigido do plano original que previa `@dfe-agent/`), TypeScript compilado (D3), base em `~/.dfe-agent/dfe.db` (D4), download via GitHub Releases (D5), modelo `Xenova/paraphrase-multilingual-MiniLM-L12-v2` (D6), paridade Py↔Node validada (gate D.7, mean=0.991), install opt-in (D8), SemVer independente tag `packages-v*.*.*` (D9), compatibilidade Node 24 LTS (D10 — resolvido no round 11 com `winget install OpenJS.NodeJS.LTS`).

- [x] **Code review automatizado (general agent, code-reviewer nao inviavel por Sprint 9 follow-up)** (gate Fase 4). Reportou 6 BLOQUEANTE + 20 IMPORTANTE + 14 SUGESTAO. Round 1 de correcao: 6/6 BLOQUEANTE + 10/20 IMPORTANTE resolvidos. Restantes (10 IMPORTANTE + 14 SUGESTAO) documentados em `PLAN_SPRINT14.md` para follow-up Sprint 15+ (drift com Py em `src/query/constants.py`, testes estruturais vs comportamentais, etc).

- [x] **Suite verde final: 50/55 testes do pacote passam (5 comportamentais skipados em CI por bug conhecido Node 22 + better-sqlite3 cleanup)** + **761 pytest no DFe-Agent root passam** (zero regressao). Paridade embeddings Py↔Node validada com cosine=0.991 >= 0.99 gate D.7.

- [x] **FOLLOW-UPS Sprint 15+** (documentados em `.opencode/rag/knowledge/2026-08-26-dev-sprint14-npm-package.md`):
   1. Re-habilitar `--provenance` (ja' re-habilitado no round 17 via Trusted Publisher)
   2. Re-habilitar Windows na CI matrix (apos upgrade Node ou better-sqlite3)
   3. Re-habilitar pytest regression (investigar qual teste quebra o gate)
   4. Implementar `dfe-agent build` em Node (regenerar base localmente)
   5. Adicionar `npm login` local + `npm publish` ao CI (substituir workflow atual)
   6. Criar versao bash do `smoke-test.ps1` (CI Linux)
   7. Port `src/collector` para Node (eliminar dependencia Python no DFe-Agent root)

## Decisoes resolvidas (Sprint 15)

- [x] **`paths.ts` central como fonte UNICA de verdade para paths do pacote `@wiati/dfe-agent`** (BLOQUEANTE B15.A — Bug A). Pre-Sprint 15, `src/query/index.ts:64-68` (correto), `src/commands/update.ts:47-52` (errado) e `src/commands/status.ts:20-24` (errado) implementavam `resolveBaseDir` independentemente. A decisao D4 de Sprint 14 (`~/.dfe-agent/dfe.db` + override `DFE_AGENT_BASE_DIR`) estava documentada mas 2 das 3 implementacoes esqueceram o `resolve(..., ".dfe-agent")`, quebrando o fluxo `update && query` end-to-end no Windows (`Cannot open database because the directory does not exist`). Correcao: novo `packages/dfe-agent/src/paths.ts` com `resolveBaseDir`, `resolveDbPath`, `resolveCacheDbPath`; os 3 entrypoints agora importam da fonte unica. Gate novo: `packages/dfe-agent/tests/query/paths.test.ts` (7 testes estruturais — 3 entrypoints + 3 paths.ts + 2 gate Bug B). Sintoma user: `npx dfe-agent query "O que e a NF-e?"` quebrava apos `npx dfe-agent update`. Knowledge: `.opencode/rag/knowledge/2026-08-27-dev-bug-dfe-agent-runtime-path-and-cache.md`.

- [x] **`QueryCache` desacoplado do handle readonly do `dfe.db`** (BLOQUEANTE B15.B — Bug B). `src/query/cache.ts` aceitava `handle: BetterSqlite3Database` no construtor e escrevia `CREATE TABLE IF NOT EXISTS query_cache` no mesmo handle aberto com `readonly: true` em `src/query/index.ts:153`, gerando `attempt to write a readonly database` (SQLITE_READONLY) em toda busca semantica/hibrida. Correcao: construtor agora recebe `baseDir: string` e abre **propria conexao** read-write em `<baseDir>/cache.db` (canonica via `resolveCacheDbPath` em `paths.ts`). `query/index.ts` agora chama `new QueryCache(resolveBaseDir(opts.baseDirOverride))`. Adicionado `close()` idempotente (gate contra destructor better-sqlite3 cleanup-hook crash). Gate estrutural cross-platform em `tests/query/paths.test.ts` ("cache.ts importa resolveCacheDbPath" + "query/index.ts passa baseDir"); gate comportamental em `tests/query/cache.test.ts` (skip em Windows Node >=22 + CI). Plano original: `PLAN_SPRINT14.md` Follow-ups #6 (Windows smoke-test) — atende parcialmente.

- [x] **Gate CI estendido para auto-detectar Windows + Node >= 22 no `tests/query/cache.test.ts`** (SUGESTAO code-review aplicada). Pre-Sprint 15, o guard `if (CI)` skipava apenas em `CI=true` ou `DFE_AGENT_SKIP_NATIVE_TESTS=1`. Bug pre-existente `RemoveEnvironmentCleanupHook` (Documentado em `2026-08-26-dev-sprint14-npm-package.md:104`) tambem reproduz em Windows + Node >= 22 (nao so' Linux + Node 22 como Sprint 14 documentou). Adicionado `IS_WIN_NODE_NATIVE_BUG = process.platform === "win32" && NODE_MAJOR >= 22` ao calculo de CI. Cobertura cross-platform sem skip manual. A causa raiz do bug better-sqlite3 esta documentada como FOLLOW-UP #1 Sprint 15+ (atualizar `better-sqlite3` resolve issue #336 WillBrennan).

- [x] **Suite verde final Sprint 15**: `npm test` no `packages/dfe-agent/`: **60 passed / 0 fail / 2 skipped (gate nativo) / 62 total**. `pytest tests/ --cov=src --cov-branch --cov-fail-under=80` no DFe-Agent root: **761 passed / 1 skipped (CONFAZ pre-existente) / cobertura 85.07%** (baseline Sprint 14 era 85.11%, -0.04pp margin). `tsc --noEmit`: sem erros. Code review (subagent `general`, code-reviewer nao inviavel): 1 BLOQUEANTE / 0 IMPORTANTE / 6 SUGESTAO round 1 -> 0/0/1 round 2 (1 SUGESTAO residual = redundancia `resolveBaseDir` vs `resolveDbPath` em update.ts mantida como NAO-aplicada por decisao humana). Pipeline `/bug`: 1 iteracao do loop corretivo.

- [x] **FOLLOW-UPS Sprint 16+** (documentados em `.opencode/rag/knowledge/2026-08-27-dev-bug-dfe-agent-runtime-path-and-cache.md`):
   1. Re-habilitar `tests/query/cache.test.ts` (comportamental) em Windows + Node >= 22 via upgrade `better-sqlite3` (issue #336 WillBrennan).
   2. Criar `tests/integration/test_query_e2e_readonly.test.ts` (gate E2E end-to-end `update && query` cross-platform; setup pesado, adiado deste `/bug`).
   3. Estender `tests/e2e/smoke-test.ps1` para incluir `query` alem de `install --auto-setup && status`.
   4. Aplicar SUGESTAO 2 residual de `update.ts` (substituir `baseDir = opts.baseDirOverride ?? resolveBaseDir()` por `dirname(resolveDbPath(...))`).
   5. Fechar Sprint 14 FOLLOW-UP #3 ("Re-habilitar pytest regression") — agora identificado: `test_opencode_init_db_creates_db_in_opencode_rag` quebra por `better_sqlite3.node` Node v22 (gate alinhado com FOLLOW-UP #1 deste sprint).

## Decisoes resolvidas (Sprint 16)

- [x] **`vectorSearch`/`ftsSearch`/`hydrateChunks` casados ao schema Py real** (BLOQUEANTE B16.1 — Bug C). Pre-Sprint 16, o pacote `@wiati/dfe-agent` v0.1.0-v0.1.3 usava nomes de coluna antigos (`chunk_id`, `doc_id`) e referenciava tabela `chunks` que **nao existe** no schema Py desde Sprint 12. Py produz `vec_chunks(document_id, chunk_index)` + `fts_chunks(document_id, chunk_index)` + sidecar `chunk_metadata` com chave composta. Sintoma: `npx dfe-agent query "..."` quebrava com `"no such column: chunk_id"`. Fix: aliases SQL em `src/query/vectorSearch.ts` + `src/query/ftsSearch.ts` (`SELECT chunk_index AS chunk_id, document_id AS doc_id FROM ...`); `hydrateChunks` em `src/query/index.ts` refatorado para `FROM vec_chunks vc JOIN documents d ON d.id = vc.document_id WHERE (vc.document_id, vc.chunk_index) IN ((?,?),(?,?), ...)`. Gates novos: `tests/query/vectorSearch.test.ts` (4 testes), `tests/query/index.test.ts` (2 testes, BEHAVIORAL skipado Windows+Node>=22); `tests/query/ftsSearch.test.ts::sanitizeQuery BEHAVIORAL` migrado para schema Py real (gate anti-regressao). Publica como v0.1.4 (PATCH bump — API publica inalterada via aliases SQL). Code review (subagent `general`): 0 BLOQUEANTE / 0 IMPORTANTE / 3 SUGESTAO round 1 -> 0/0/0 round 2. Knowledge: `.opencode/rag/knowledge/2026-08-27-dev-bug-dfe-agent-schema-drift.md`. Plano: `PLAN_SPRINT16.md` (a criar).

- [x] **`tests/e2e/smoke-test.ps1` NAO cobre `query` end-to-end** (registro de gap; **FOLLOW-UP Sprint 17+ #1**). Confirmado em auditoria Sprint 16 Bug C: o smoke test so' executa `install --auto-setup && status`. Por isso, o drift Py<->Node passou batido por Sprint 14+15 — gate ausente. Fix proposto: estender `smoke-test.ps1` para incluir `query` (com query dummy + assertiva de NAO conter "no such column" / "attempt to write" / "Cannot open database"). Adicionalmente: gate Linux via versao bash do `smoke-test.ps1` (FOLLOW-UP Sprint 14 #6 parcialmente atendido por este gap).

- [x] **`tests/integration/test_query_e2e_readonly.test.ts` registrado como follow-up Sprint 17+** (FOLLOW-UP Sprint 15+ #2). Gate E2E completo com `dfe.db` real em `tmp_path`: cria DB com schema Py (vec0 + fts5 + documents + chunk_metadata), popula com 2 docs × 3 chunks, roda `search()` com query dummy, valida `{answer, sources[]}`. Setup pesado (depende da upgrade de `better-sqlite3` para sair do gate nativo Windows+Node>=22); adiado para Sprint 17.

- [x] **Suite verde final Sprint 16**: `npm test` no `packages/dfe-agent/`: **64 passed / 0 fail / 5 skipped (gate nativo) / 69 total** (Sprint 15 fechou em 60/2/62; +9 testes gate Bug C). `pytest tests/ --cov=src --cov-branch --cov-fail-under=80` no DFe-Agent root: **761 passed / 1 skipped (CONFAZ pre-existente) / cobertura 85.07%** (baseline mantido). `tsc --noEmit`: sem erros. `npm run drift-check`: nenhum drift. Code review (subagent `general`, code-reviewer nao inviavel por Sprint 9 follow-up): 0/0/3 round 1 -> 0/0/0 round 2. Pipeline `/bug`: 1 iteracao do loop corretivo.

## Decisoes resolvidas (Sprint 17)

- [x] **Embeddings Py normalizados (Opção A: re-ingest)** (BLOQUEANTE B17.1 — Bug D). Pre-Sprint 17, `src/indexer/embeddings.py:213` embedava chunks SEM `normalize_embeddings=True`, salvando vetores com norm ~2.6-3.3 em `storage/dfe.db`. Node (`packages/dfe-agent/src/query/embedder.ts:66`) normaliza para norm = 1.0. Resultado: L2 distance > 1.0 sempre, score < 0.5, `hasSufficientEvidence` sempre false, `NO_EVIDENCE_MESSAGE` para qualquer query. Gate D.7 validava apenas sentenças curtas; drift em chunks longos não detectado. Fix Opção A (Sprint 17, aprovado por Andrews): adicionar `normalize_embeddings=True` em `embeddings.py:213` + re-ingerir 28.182 chunks (448 documentos) + ajustar `MIN_RELEVANCE_SCORE` de 0.5 para 0.3 (gate drift residual Py↔Node em chunks longos: Py usa attention_mask no mean pooling, Node usa `@xenova/transformers` sem attention_mask explicito; gate D.7 cobre apenas sentenças curtas). Gates: smoke test end-to-end com `vectorSearch` direto + CLI `dfe-agent query` em 4 queries (2 com termos técnicos específicos retornam chunks reais com score > 0.3; 2 genéricas retornam `NO_EVIDENCE_MESSAGE` corretamente). Publica como v0.1.5 (PATCH bump — API publica inalterada). Code review aplicado via `/bug` canonico. Plano: `PLAN_SPRINT17.md`. Knowledge: `.opencode/rag/knowledge/2026-08-27-dev-bug-dfe-agent-embedding-normalization.md` (a criar). FOLLOW-UPS Sprint 18+:
  ...
  3. Re-habilitar publish-npm CI (remover `if: false` + `needs: test`) — gate FOLLOW-UP Sprint 14 #5. **SUBSTITUIDO em Sprint 18**: CI foi removido; publicacao agora via slash command `/deploy --npm`.
  4. Estender `tests/e2e/smoke-test.ps1` para incluir `query` (gate FOLLOW-UP Sprint 17+ #1).
  5. Upgrade `better-sqlite3` (issue #336 WillBrennan) para destravar gate nativo Win+Node>=22 (gate FOLLOW-UP Sprint 14 #2).
  6. Adicionar `--top-k` no CLI `dfe-agent query` para inspeção de hits brutos (debug).

- [x] **Suite verde final Sprint 17**: `npm test` no `packages/dfe-agent/`: **64 passed / 0 fail / 5 skipped (gate nativo) / 69 total** (baseline mantido). `pytest tests/ --cov=src --cov-branch --cov-fail-under=80` no DFe-Agent root: **761 passed / 1 skipped (CONFAZ) / cobertura 85.07%** (baseline mantido). Smoke test end-to-end: `vectorSearch` direto retornou hits com score 0.31-0.39 (acima de MIN_RELEVANCE_SCORE=0.3); CLI `dfe-agent query "nota tecnica 2024.001"` retornou JSON com `answer` + `sources` (chunks reais da base re-ingerida). `tsc --noEmit`: sem erros. `npm run drift-check`: nenhum drift. Code review aplicado via `/bug` (gate duplo). Pipeline `/bug`: 1 iteracao do loop corretivo.

## Decisoes resolvidas (Sprint 18)

- [x] **Remocao completa do CI (GitHub Actions)** (BLOQUEANTE B18.1). Os 3 workflows em `.github/workflows/` foram **removidos**: `test-npm-package.yml` (CI matrix ubuntu × Node 20.x/22.x; pytest-regression `if: false` desde Sprint 14 round 12; E2E smoke `if: false`), `publish-npm.yml` (publish job `if: false` desde Sprint 14 round 24 — user prefere manual), `publish-base.yml` (publicava `storage/dfe.db.gz` no GitHub Releases em tag `v*.*.*`). Diagnostico via API GitHub em 2026-08-27: dos 4 runs mais recentes, **3 falharam** (`publish-base #5`, `publish-npm.yml #40`, `publish-npm.yml #41`); apenas `publish-base #6` (com o SHA local `78d09c1`) passou. CI degradado, sem valor residual. Diretorio `.github/workflows/` apagado. Gate anti-regressao em `tests/integration/test_no_legacy_ci.py` (3 testes parametrizados).

- [x] **Agente `@deployer` criado** (BLOQUEANTE B18.2). Subagent primario (`mode: primary`, slug `deployer`) em `.opencode/agent/deployer.md`. **Unico autorizado** a fazer `git push`, `git tag`, `npm publish`, `gh release create` — gate enforced via `permission.edit: deny` + hook `.opencode/hooks/deployer/pre_tool_use.py` com **allow list explicita** (git/npm/gh/npx dfe-agent/RAG search+embed) + block list defensiva (rm -rf, sed -i, >, curl, wget, pip, pipeline RAG). PostToolUse observer lightweight (NAO roda pytest); Stop exit 0 sem pytest e sem RAG capture (deployer NAO aprende). Dispatch via `.opencode/plugin/agent-hooks.ts` (entrada `deployer` adicionada a `AGENTS` e `RECOGNIZED_AGENT_SLUGS`). **142 testes novos Sprint 18** distribuidos em 8 arquivos:
  - `tests/unit/test_deployer_agent_definition.py`: 25 testes estruturais.
  - `tests/unit/hooks/test_deployer_pre_tool_use.py`: 75 testes bash allow/block.
  - `tests/unit/hooks/test_deployer_post_tool_use.py`: 4 testes.
  - `tests/unit/hooks/test_deployer_stop.py`: 4 testes.
  - `tests/unit/hooks/test_deployer_payload_detection.py`: 8 testes (consolida `_AGENT_HINTS` + `suites_for_path`).
  - `tests/integration/test_no_legacy_ci.py`: 7 testes gate anti-regressao.
  - `tests/integration/test_deployer_plugin_dispatch.py`: 11 testes (dispatch + py_compile + tsx).
  - `tests/unit/test_commands_definitions.py`: 8 testes novos para `/deploy`.

- [x] **Slash command `/deploy`** (BLOQUEANTE B18.3). Definido em `.opencode/command/deploy.md` com `agent: deployer`. 4 modos canonicos: **bare** (`/deploy` -> `git push`), **tag** (`/deploy --tag vX.Y.Z` -> push + tag + push tag), **npm** (`/deploy --npm` -> push + tag `v0.0.1-sprint<N>` + `npm publish --access public --provenance`), **release** (`/deploy --release vX.Y.Z [notas]` -> push + tag + `gh release create`). Gate humano explicito (Fase 3.5) antes de cada acao destrutiva (`--npm`, `--release`, `--tag` com overwrite, `--force`, delete de tag): comando imprime ACAO DESTRUTIVA DETECTADA e aguarda resposta "sim, executar". RAG antes/depois padronizado (`search.ts -a deployer` na Fase 0; `embed.ts --file` sincrono na Fase 4). Cobertura via 8 testes parametrizados em `tests/unit/test_commands_definitions.py` (gate 2.6).

- [x] **`@dev` NAO edita git/npm** (gate anti-regressao, defesa em profundidade). `dev/pre_tool_use.py` ja' bloqueia `git push`, `gh pr create`, `gh release` desde Sprint 10 (linha 56-60). `code-reviewer` tem `permission.edit: deny` + hook `pre_tool_use_bash.py` que bloqueia `git push`. Confirmado por teste Sprint 9 (`tests/unit/hooks/test_code_reviewer_pre_tool_use_bash.py`) e Sprint 10 (`tests/unit/hooks/test_dev_pre_tool_use.py`). A introducao do `@deployer` nao enfraquece esses gates — eles continuam validos e sao a primeira linha de defesa.

- [x] **Helpers `_lib/` atualizados**. `_lib/payload.py::_AGENT_HINTS` agora reconhece `deployer` com lookahead negativo `(?!ed|ing|ment)` (evita match em `deployed`/`deploying`/`deployment`). `_lib/test_runner.py::suites_for_path` ganhou branch `agent == "deployer"` retornando suite vazia (deployer NAO roda pytest). 8 testes em `tests/unit/hooks/test_deployer_payload_detection.py`.

- [x] **Suite verde final Sprint 18**: `npm test` no `packages/dfe-agent/`: **64 passed / 0 fail / 5 skipped (gate nativo) / 69 total** (baseline Sprint 17 mantido). `pytest tests/ --cov=src --cov-branch --cov-fail-under=80` no DFe-Agent root: **890 passed / 1 skipped (CONFAZ) / cobertura 84.99%** (baseline Sprint 17 era 85.07%, -0.08pp margin; gate 80% verde). `tsc --noEmit`: sem erros. Code review (subagent `general`, code-reviewer nao inviavel por Sprint 9 follow-up): 1 BLOQUEANTE / 3 IMPORTANTE / 3 SUGESTAO round 1 -> 0/0/0 round 2 (loop corretivo aplicado em 1 iteracao). Pipeline `/feature`: 1 iteracao do loop corretivo. Knowledge: `.opencode/rag/knowledge/2026-08-27-dev-feature-deployer-agent.md` (Fase 6).

- [x] **FOLLOW-UPS Sprint 19+** (documentados em `.opencode/rag/knowledge/2026-08-27-dev-feature-deployer-agent.md`):
